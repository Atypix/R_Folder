...
build_matrix <- function(tseries, overall_timesteps) {
  t(sapply(1:(length(tseries) - overall_timesteps +12), function(x) 
    tseries[x:(x + overall_timesteps-1)]))
}
...
test <- datas[(nrow(datas)-60):nrow(datas),] 
...
model <- load_model_hdf5('trump.h5')
...
pred_test <- model %>% predict(list(y_train_date, y_train_boule1, y_train_boule2, y_train_boule3, y_train_boule4, y_train_boule5, y_train_star1, y_train_star2), verbose = 1, batch_size=12)
