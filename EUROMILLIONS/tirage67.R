...
scale_boules <- function(boule1, 
  ...
  std_boule1_validation = (boule1_validation - min(x)) / (max(x) - min(x))
  std_boule2_validation = (boule2_validation - min(x)) / (max(x) - min(x))
  std_boule3_validation = (boule3_validation - min(x)) / (max(x) - min(x))
  std_boule4_validation = (boule4_validation - min(x)) / (max(x) - min(x))
  std_boule5_validation = (boule5_validation - min(x)) / (max(x) - min(x))
  
  
  scaled_boule1_validation = std_boule1_validation * (fr_max - fr_min) + fr_min
  scaled_boule2_validation = std_boule2_validation * (fr_max - fr_min) + fr_min
  scaled_boule3_validation = std_boule3_validation * (fr_max - fr_min) + fr_min
  scaled_boule4_validation = std_boule4_validation * (fr_max - fr_min) + fr_min
  scaled_boule5_validation = std_boule5_validation * (fr_max - fr_min) + fr_min
  ...
  return( list(scaled_boule1 = as.vector(scaled_boule1), 
               ...
               scaled_boule1_validation = as.vector(scaled_boule1_validation), 
               scaled_boule2_validation = as.vector(scaled_boule2_validation),
               scaled_boule3_validation = as.vector(scaled_boule3_validation),
               scaled_boule4_validation = as.vector(scaled_boule4_validation),
               scaled_boule5_validation = as.vector(scaled_boule5_validation),
               
               scaler= c(min =min(x), max = max(x))) )
}

scale_star <- function(star1, star2, star1_test, star2_test, star1_validation, star2_validation, feature_range = c(0,1)) {
  ...
  std_star1_validation = (star1_validation - min(x)) / (max(x) - min(x))
  std_star2_validation = (star2_validation - min(x)) / (max(x) - min(x))
  ...
  scaled_star1_validation = std_star1_validation * (fr_max - fr_min) + fr_min
  scaled_star2_validation = std_star2_validation * (fr_max - fr_min) + fr_min
  ...
  return( list(scaled_star1 = as.vector(scaled_star1), 
               ...
               scaled_star1_validation = as.vector(scaled_star1_validation),
               scaled_star2_validation = as.vector(scaled_star2_validation),
               scaler= c(min =min(x), max = max(x))) )
  
}

scale_date <- function(train, test, validation, feature_range = c(0,1)) {
  ...
  return( list(scaled_train = as.vector(scaled_train), 
               ...
               scaled_validation = as.vector(scaled_test) ,
               scaler= c(min =min(x), max = max(x))) )
...
row.names(validation) <- seq(1:nrow(validation))
...
boule1_validation <- validation$boule_1
boule2_validation <- validation$boule_2
boule3_validation <- validation$boule_3
boule4_validation <- validation$boule_4
boule5_validation <- validation$boule_5
...
star1_validation <- validation$etoile_1
star2_validation <- validation$etoile_2

ScaledDate <- scale_date(as.data.frame(date_train), as.data.frame(date_test), as.data.frame(date_validation), c(0,1))
x_train_date <- ScaledDate$scaled_train[,1]
x_test_date <- ScaledDate$scaled_test[,1]
x_validation_date <- ScaledDate$scaled_validation[,1]
...
ScaledBoule <- scale_boules(as.data.frame(boule1), 
                            ...
                            as.data.frame(boule1_validation), 
                            as.data.frame(boule2_validation),
                            as.data.frame(boule3_validation),
                            as.data.frame(boule4_validation),
                            as.data.frame(boule5_validation),
                            c(0,1))
...
x_train_boule1_validation <- ScaledBoule$scaled_boule1_validation[,1]
x_train_boule2_validation <- ScaledBoule$scaled_boule2_validation[,1]
x_train_boule3_validation <- ScaledBoule$scaled_boule3_validation[,1]
x_train_boule4_validation <- ScaledBoule$scaled_boule4_validation[,1]
x_train_boule5_validation <- ScaledBoule$scaled_boule5_validation[,1]
...
ScaledStar <- scale_star(as.data.frame(star1), 
                         ...
                         as.data.frame(star1_validation),
                         as.data.frame(star2_validation),
                         c(0,1))
...
x_validation_star1 <- ScaledStar$scaled_star1_validation[,1]
x_validation_star2 <- ScaledStar$scaled_star2_validation[,1]
...
validation_matrix_date <- build_matrix(x_validation_date, n_timesteps + n_predictions)
...
train_matrix_boule1_validation <- build_matrix(x_train_boule1_validation, n_timesteps + n_predictions)
train_matrix_boule2_validation <- build_matrix(x_train_boule2_validation, n_timesteps + n_predictions)
train_matrix_boule3_validation <- build_matrix(x_train_boule3_validation, n_timesteps + n_predictions)
train_matrix_boule4_validation <- build_matrix(x_train_boule4_validation, n_timesteps + n_predictions)
train_matrix_boule5_validation <- build_matrix(x_train_boule5_validation, n_timesteps + n_predictions)
...
train_matrix_star1_validation <- build_matrix(x_validation_star1, n_timesteps + n_predictions)
train_matrix_star2_validation <- build_matrix(x_validation_star2, n_timesteps + n_predictions)
...
X_validation_date <- validation_matrix_date[, 1:n_timesteps]
y_validation_date <- validation_matrix_date[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_date <- X_validation_date[1:(nrow(X_validation_date) %/% batch_size * batch_size),]
y_validation_date <- y_validation_date[1:(nrow(y_validation_date) %/% batch_size * batch_size),]
...
X_validation_boule1 <- train_matrix_boule1_validation[, 1:n_timesteps]
y_validation_boule1 <- train_matrix_boule1_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_boule1 <- X_validation_boule1[1:(nrow(X_validation_boule1) %/% batch_size * batch_size),]
y_validation_boule1 <- y_validation_boule1[1:(nrow(y_validation_boule1) %/% batch_size * batch_size),]
...
X_validation_boule2 <- train_matrix_boule2_validation[, 1:n_timesteps]
y_validation_boule2 <- train_matrix_boule2_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_boule2 <- X_validation_boule2[1:(nrow(X_validation_boule2) %/% batch_size * batch_size),]
y_validation_boule2 <- y_validation_boule2[1:(nrow(y_validation_boule2) %/% batch_size * batch_size),]
...
X_validation_boule3 <- train_matrix_boule3_validation[, 1:n_timesteps]
y_validation_boule3 <- train_matrix_boule3_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_boule3 <- X_validation_boule3[1:(nrow(X_validation_boule3) %/% batch_size * batch_size),]
y_validation_boule3 <- y_validation_boule3[1:(nrow(y_validation_boule3) %/% batch_size * batch_size),]
...
X_validation_boule4 <- train_matrix_boule4_validation[, 1:n_timesteps]
y_validation_boule4 <- train_matrix_boule4_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_boule4 <- X_validation_boule4[1:(nrow(X_validation_boule4) %/% batch_size * batch_size),]
y_validation_boule4 <- y_validation_boule4[1:(nrow(y_validation_boule4) %/% batch_size * batch_size),]
...
X_validation_boule5 <- train_matrix_boule5_validation[, 1:n_timesteps]
y_validation_boule5 <- train_matrix_boule5_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_boule5 <- X_validation_boule5[1:(nrow(X_validation_boule5) %/% batch_size * batch_size),]
y_validation_boule5 <- y_validation_boule5[1:(nrow(y_validation_boule5) %/% batch_size * batch_size),]
...
X_validation_star1 <- train_matrix_star1_validation[, 1:n_timesteps]
y_validation_star1 <- train_matrix_star1_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_star1 <- X_validation_star1[1:(nrow(X_validation_star1) %/% batch_size * batch_size),]
y_validation_star1 <- y_validation_star1[1:(nrow(y_validation_star1) %/% batch_size * batch_size),]
...
X_validation_star2 <- train_matrix_star2_validation[, 1:n_timesteps]
y_validation_star2 <- train_matrix_star2_validation[, (n_timesteps + 1):(n_timesteps * 2)]
...
X_validation_star2 <- X_validation_star2[1:(nrow(X_validation_star2) %/% batch_size * batch_size),]
y_validation_star2 <- y_validation_star2[1:(nrow(y_validation_star2) %/% batch_size * batch_size),]


X_train_date <-  reshape_X_3d(X_train_date)
y_train_date <-  reshape_X_3d(y_train_date)
X_test_date <-  reshape_X_3d(X_test_date)
y_test_date <-  reshape_X_3d(y_test_date)
X_validation_date <-  reshape_X_3d(X_validation_date)
y_validation_date <-  reshape_X_3d(y_validation_date)
...
X_validation_boule1 <- reshape_X_3d(X_validation_boule1)
y_validation_boule1 <- reshape_X_3d(y_validation_boule1)
...
X_validation_boule2 <- reshape_X_3d(X_validation_boule2)
y_validation_boule2 <- reshape_X_3d(y_validation_boule2)
...
X_validation_boule3 <- reshape_X_3d(X_validation_boule3)
y_validation_boule3 <- reshape_X_3d(y_validation_boule3)
...
X_validation_boule4 <- reshape_X_3d(X_validation_boule4)
y_validation_boule4 <- reshape_X_3d(y_validation_boule4)
...
X_validation_boule5 <- reshape_X_3d(X_validation_boule5)
y_validation_boule5 <- reshape_X_3d(y_validation_boule5)
...
X_validation_star1 <- reshape_X_3d(X_validation_star1)
y_validation_star1 <- reshape_X_3d(y_validation_star1)
...
X_validation_star2 <- reshape_X_3d(X_validation_star2)
y_validation_star2 <- reshape_X_3d(y_validation_star2)
...
outputLSTM_boule1 <- inputsLSTM_boule1  %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.0001, l2=0.001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
outputLSTM_boule2 <- inputsLSTM_boule2  %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
  layer_dropout(rate = 0.001) %>% 
  layer_reshape(c(n_timesteps, 1))%>% 
...
#   layer_lstm (units = 1000, input_shape = c(n_timesteps, 1)) %>%  
...
combined_boule1 <- layer_concatenate(c(outputLSTM_boule1, outputLSTM_boule2, outputLSTM_boule3, outputLSTM_boule4, outputLSTM_boule5), name='concatenate_boule1') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_boule2 <- layer_concatenate(c(outputLSTM_boule1, outputLSTM_boule2, outputLSTM_boule3, outputLSTM_boule4, outputLSTM_boule5), name='concatenate_boule2') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_boule3 <- layer_concatenate(c(outputLSTM_boule1, outputLSTM_boule2, outputLSTM_boule3, outputLSTM_boule4, outputLSTM_boule5), name='concatenate_boule3') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_boule4 <- layer_concatenate(c(outputLSTM_boule1, outputLSTM_boule2, outputLSTM_boule3, outputLSTM_boule4, outputLSTM_boule5), name='concatenate_boule4') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_boule5 <- layer_concatenate(c(outputLSTM_boule1, outputLSTM_boule2, outputLSTM_boule3, outputLSTM_boule4, outputLSTM_boule5), name='concatenate_boule5') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_star1 <- layer_concatenate(c(outputLSTM_star1, outputLSTM_star2), name='concatenate_star1') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
combined_star2 <- layer_concatenate(c(outputLSTM_star1, outputLSTM_star2), name='concatenate_star2') %>%
  layer_lstm (units = 100, input_shape = c(n_timesteps, 1)) %>% 
  layer_dense(n_timesteps, kernel_regularizer=regularizer_l1_l2(l1=0.00001, l2=0.0001),
              bias_regularizer=regularizer_l2(0.0001),
              activity_regularizer=regularizer_l2(0.00001)) %>%
...
history <- model %>% fit(x = list( X_train_boule1, X_train_boule2, X_train_boule3, X_train_boule4, X_train_boule5, X_train_star1, X_train_star2),
                         y = list(y_train_boule1, y_train_boule2, y_train_boule3, y_train_boule4, y_train_boule5, y_train_star1, y_train_star2),
                         epochs = epochs,
