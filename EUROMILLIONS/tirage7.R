...
test <- datas[(nrow(datas)-4):nrow(datas),] 
...
outputLSTM_boule1 <- inputsLSTM_boule1  %>%
  layer_lstm (units = 100, name='lstm_0_boule1', return_sequences = TRUE) %>% 
...
outputLSTM_boule2 <- inputsLSTM_boule2  %>%
  layer_lstm (units = 100, name='lstm_0_boule2', return_sequences = TRUE) %>% 
...
outputLSTM_boule3 <- inputsLSTM_boule3  %>%
  layer_lstm (units = 100, name='lstm_0_boule3', return_sequences = TRUE) %>% 
...
outputLSTM_boule4 <- inputsLSTM_boule4  %>%
  layer_lstm (units = 100, name='lstm_0_boule4', return_sequences = TRUE) %>% 
...
outputLSTM_boule5 <- inputsLSTM_boule5  %>%
  layer_lstm (units = 100, name='lstm_0_boule5', return_sequences = TRUE) %>% 
...
outputLSTM_star1 <- inputsLSTM_star1  %>%
  layer_lstm (units = 100, name='lstm_0_star1', return_sequences = TRUE) %>% 
...
outputLSTM_star2 <- inputsLSTM_star2  %>%
  layer_lstm (units = 100, name='lstm_0_star2', return_sequences = TRUE) %>% 
...
#list(X_test_date, X_test_boule1, X_test_boule2, X_test_boule3, X_test_boule4, X_test_boule5, X_test_star1, X_test_star2
...
forecast_future_values <- function(model, x, steps) {
  ...
  for (i in seq_len(steps)) {
    ...
    next_value <- model %>%
      predict(x_reshaped, verbose = 1, batch_size=batch_size)
