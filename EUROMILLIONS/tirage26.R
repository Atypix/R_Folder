...
build_matrix <- function(tseries, overall_timesteps) {
  t(sapply(1:(length(tseries) - overall_timesteps +2), function(x) 
    tseries[x:(x + overall_timesteps)]))
}
...
test <- datas[(nrow(datas)-500):nrow(datas),] 
...
outputLSTM_boule1 <- inputsLSTM_boule1  %>%
  layer_lstm (units = 100, name='lstm_0_boule1', activation='linear') %>%
...
outputLSTM_boule2 <- inputsLSTM_boule2  %>%
  layer_lstm (units = 100, name='lstm_0_boule2', activation='linear') %>%
...
outputLSTM_boule3 <- inputsLSTM_boule3  %>%
  layer_lstm (units = 100, name='lstm_0_boule3', activation='linear') %>%
...
outputLSTM_boule4 <- inputsLSTM_boule4  %>%
  layer_lstm (units = 100, name='lstm_0_boule4', activation='linear') %>%
...
outputLSTM_boule5 <- inputsLSTM_boule5  %>%
  layer_lstm (units = 100, name='lstm_0_boule5', activation='linear') %>%
...
outputLSTM_star1 <- inputsLSTM_star1  %>%
  layer_lstm (units = 100, name='lstm_0_star1', activation='linear') %>%
...
outputLSTM_star2 <- inputsLSTM_star2  %>%
  layer_lstm (units = 100, name='lstm_0_star2', activation='linear') %>%
...
output_date  <- inputs_date %>%
  layer_lstm (units = 100, name='lstm_0_date', activation='linear') %>%
...
#model <- load_model_hdf5("modelnonCombine.h5")
#summary(model)
