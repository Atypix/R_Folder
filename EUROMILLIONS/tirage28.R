...
build_matrix <- function(tseries, overall_timesteps) {
  t(sapply(1:(length(tseries) - overall_timesteps +68), function(x) 
    tseries[x:(x + overall_timesteps-1)]))
}
...
#model <- load_model_hdf5('tirage26.h5')
model %>% compile(
  loss = 'mse',
  optimizer = 'adam'
)
...
names(view_test)[1] <- 'x1'
names(view_test)[2] <- 'x2'
names(view_test)[3] <- 'x3'
names(view_test)[4] <- 'x4'
names(view_test)[5] <- 'x5'
names(view_test)[6] <- 'x6'
names(view_test)[7] <- 'x7'
