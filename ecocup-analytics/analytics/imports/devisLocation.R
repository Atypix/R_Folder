  ...
  out <- tryCatch(
    {
      ...
      url <- 'http://personnel.ecocup.fr/bo2api/controllers/quotes/index.php?rent=true&start='
      
      df <- jsonlite::fromJSON(txt = paste0(url, startDate, "&end=",endDate))
      message(paste("Chargement des données Devis Location du :", startDate, "au", endDate))
      ...
      #on insere en BDD
      dbWriteTable(DB, "location", df,append=TRUE)
