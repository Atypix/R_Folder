...
rownames(d) <- NULL
datas <- cbind(date,d)
b <- ggplot(datas, aes(x = as.Date(date), y = ATO.Open))
b <- b + geom_line()
print(b)
# 
...
# 
# n_ <- round(nrow(supervised) * .85, digits = 0)
...
# y_train <- expand_dims(y_train, -1)
# 
# model <- keras_model_sequential()
# 
# model %>% layer_lstm (units = 50,
#                       input_shape = dim(x_train)[2], name='lstm_input', batch_size=32)
